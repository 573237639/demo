	...
	$("#orderadd-datagrid").datagrid({
		...
		columns : [ [
	  		...
	  		loader:function(param,success,error){
		    	...
		    	$.ajax({
		            url:"${ctx}/tb/ledger/orderadd-datagrid",
		            checkbox: "true", 
		            data:param,
		            type:"post",
		            dataType:"json",
		            ...
		            success: function(data){
		            	if(data.total == 0){
		            		$(".orderadd_view").hide();
		            		$(".orderadd_datanull").show();
		            	}else{
		            		$(".orderadd_view").show();
		            		$(".orderadd_datanull").hide();
		            	}
		                success(data);
		            }
		        });
...
<script type="text/javascript">
	//点击搜索	
	function submitForm() {
		$("#orderaddorder-search").linkbutton('disable');
		$.messager.progress();	// 显示进度条
		$("#orderadd-datagrid").datagrid('load');
		$.messager.progress("close");	// 关闭进度条
    	$("#orderaddorder-search").linkbutton('enable');
	}
		...
		<form id="ledgereditorder-from" method="post" action=""  modelAttribute="tbOrders">
				<div >
					...
					<input class="easyui-combobox combobox_input" id="numOrderStatus"　name="numOrderStatus"  style="width:100px;height:28px;" editable="false" 
						...
						&nbsp;&nbsp;&nbsp;&nbsp;
					<a href="javascript:submitForm();" id="orderaddorder-search"  iconCls="icon-search"  class="easyui-linkbutton a_button">搜	索</a>
				</div>
		</form>
		<br/>
		<div class="orderadd_view">
		<table class="easyui-datagrid" id="orderadd-datagrid" style="width:100%;" pagination="true"></table>
				</div>
			<div class="orderadd_datanull" style="display: none;text-align:center;">
				暂无结果
			</div>	
			</div>		
