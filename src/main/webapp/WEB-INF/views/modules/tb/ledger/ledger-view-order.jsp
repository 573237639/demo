...
var statusTypeList = ${statusTypeList};
	var loadGrid = function() {
		...
		$("#ledgervieworder-datagrid").datagrid({
			striped:true,
	  		checkOnSelect:true ,
	  		fitColumns:true,
			height:"auto",
			...
			{field:'dis',title:'操作',width:80,align:"center",formatter:function(value,row,index){
  	  			var btns = "";
  	  				btns += '<a href="javascript:viewOrder('+row.id+');"><font style="color:red">查看</font></a>&nbsp;'
				return btns;
  	  		}}]],
  		loader:function(param,success,error){
	    	...
	    	$.ajax({
	            url:"${ctx}/tb/ledger/ledgervieworder-datagrid",
	            checkbox: "true", 
	            data:param,
	            type:"post",
	            dataType:"json",
	            ...
	            success: function(data){
	            	if(data.total == 0){
	            		$(".ledger_order_view").hide();
	            		$(".ledger_order_datanull").show();
	            	}else{
	            		$(".ledger_order_view").show();
	            		$(".ledger_order_datanull").hide();
	            	}
	                success(data);
	            }
	        })
...
<script type="text/javascript">
	//点击搜索	
	function submitForm() {
		$("#ledgervieworder-search").linkbutton('disable');
		$.messager.progress();	// 显示进度条
		$("#ledgervieworder-datagrid").datagrid('load');
		$.messager.progress("close");	// 关闭进度条
    	$("#ledgervieworder-search").linkbutton('enable');
	}
...
<br/>
		<form id="ledgervieworder-from" method="post" action=""  modelAttribute="tbOrders">
				<div >
					...
					<input class="easyui-combobox combobox_input" id="numOrderStatus"　name="numOrderStatus"  editable="false"   style="width:100px;height:28px;"
						data-options="valueField:'value',width:200,panelHeight:'auto',textField:'label',url:'${ctx}/sys/dict/findDictTypeBy/status_type'" />
						&nbsp;&nbsp;&nbsp;&nbsp;
					<a href="javascript:submitForm();" id="ledgervieworder-search"  iconCls="icon-search"  class="easyui-linkbutton a_button">搜	索</a>
				</div>
		</form>
		<br/>
		<div class="ledger_order_view">
		<table class="easyui-datagrid" id="ledgervieworder-datagrid" 	style="width: 100%;" pagination="true">
		</table>
					</div>
			<div class="ledger_order_datanull" style="display: none;text-align:center;">
				暂无结果
			</div>
	</div>