...
<script type="text/javascript">
	...
	function saveOrder() {
		...
		//工单期限
		var dateOrderTerm = $("input[name='dateOrderTerm']").val();
		if(dateOrderTerm === ''){
			$.messager.alert("消息提醒","请选择工单期限!","warning");
			return;
		}
		//任务类型
	...
	function getOrderDate() {
		...
		for ( var data in orderDateList) {
			if (orderDate == orderDateList[data].value) {
				$("#dateOrderTermView").textbox('setValue',
						orderDateList[data].label);
				break;
			}
		}
	}
		...
		$('#numClientProvinceId').combobox(
				{
					...
					onHidePanel : function() {
						...
						var clientPid = $("#numClientProvinceId").combobox(
								'getValue');
						if (clientPid == '') {
							clientPid = 0;
						}
		...
		$('#numOrderIncidentProvinceId').combobox(
				{
					...
					onChange : function(pid) {
						...
						var orderIncPid = $("#numOrderIncidentProvinceId")
								.combobox('getValue');
						if (orderIncPid == '') {
							orderIncPid = 0;
						}
		...
		$('#numOrderContactProvinceId').combobox(
				{
					...
					onChange : function(pid) {
						...
						var orderContactPid = $("#numOrderContactProvinceId")
								.combobox('getValue');
						if (orderContactPid == '') {
							orderContactPid = 0;
						}
		...
		$('#numOrderReceiveProvinceId').combobox(
				{
					...
					onChange : function(pid) {
						...
						var orderReceivePid = $("#numOrderReceiveProvinceId")
								.combobox('getValue');
						if (orderReceivePid == '') {
							orderReceivePid = 0;
						}
...
<p />
<form id="orderadd-form" method="post" action=""
	modelAttribute="tbClients,tbOrders">
	<div style="display: none;">
		<span class="div_margin_left">工单id</span> <input name="tbOrders.id"
			value="${tbOrders.id }"> <span class="div_margin_left">客户id</span>
		<input name="tbClients.id" value="${tbClients.id }"> <input
			name="numClientId" value="${tbClients.id }">
	</div>
	
	<div>
		<span class="div_margin_left">工单流水号</span> <input
			class="easyui-textbox textbox_input" name="vacOrderSerial"
			value="${tbOrders.vacOrderSerial }" readonly="readonly"> <span
			class="div_margin_left">来电号码</span> <input
			class="easyui-textbox textbox_input" name="vacOrderNumber"
			id="vacOrderNumber" value="${tbOrders.vacOrderNumber }"
			readonly="readonly" />
	</div>
	<br />
	<div>
		<span class="div_margin_left">制单员工号</span> <input
			class="easyui-textbox textbox_input" name="vacOrderAgentCode"
			value="${tbOrders.vacOrderAgentCode }" readonly="readonly"> <span
			class="div_margin_left">制单员姓名</span> <input
			class="easyui-textbox textbox_input" name="vacOrderAgentName"
			value="${tbOrders.vacOrderAgentName }" readonly="readonly">
	</div>
	...
	<div>
		...
		<input class="easyui-textbox textbox_input" name="vacClientName"
			value="${tbClients.vacClientName}"  required="required"  validType="length[0,20]" >
		<span class="div_margin_left  div_color_red">* </span><span>性别</span>
		<c:forEach items="${genderList}" var="m">
			<span class="redio_span"> <input class="redio_input"
				type="radio" id="numClientGender${m.value }" name="numClientGender"
				value="${m.value }"
				<c:if test="${tbClients.numClientGender == m.value}">checked="checked"</c:if> />${m.label }
			</span>
		</c:forEach>
	</div>
	<br />
	<div>
		...
		<c:forEach items="${typeList}" var="m">
			<span class="redio_span"> <input class="redio_input"
				type="radio" id="numClientType${m.value }" name="numClientType"
				value="${m.value }"
				<c:if test="${tbClients.numClientType == m.value}">checked="checked"</c:if> />${m.label }
			</span>
		</c:forEach>
	</div>
	<br />
	<div>
		...
		<c:forEach items="${papersTypeList}" var="m">
			<span class="redio_span"> <input class="redio_input"
				type="radio" id="zjType${m.value }" name="zjType"
				value="${m.value }"
				<c:if test="${tbClients.zjType == m.value}">checked="checked"</c:if> />${m.label }
			</span>
		</c:forEach>
		<span class="div_margin_left " >证件号码</span>
		<input class="easyui-textbox textbox_input" id="zjCode" name="zjCode"
			value="${tbClients.zjCode}"  validType="length[0,20]" >
	</div>
	<br />
	<div>
		 <span
			class="div_margin_left  div_color_red">* </span><span>业务类型</span> <input id="vacOrderBusinessType" name="vacOrderBusinessType"
			value="${tbOrders.vacOrderBusinessType }"
	...
	<div>
		...
		<input class="easyui-textbox textbox_input" name="vacOrderTitle"
			value="${tbOrders.vacOrderTitle }"  required="required"  validType="length[0,20]"/> <span
			class="div_margin_left  div_color_red">* </span><span>工单类型</span>
		<c:forEach items="${orderTypeList}" var="m">
			<span class="redio_span"> <input class="redio_input"
				type="radio" id="orderType${m.value }" name="vacOrderType"
				value="${m.value }"
				<c:if test="${tbOrders.vacOrderType == m.value}">checked="checked"</c:if>
				onChange="getOrderDate()" />${m.label }
			</span>
		</c:forEach>
		<span class="div_margin_left  div_color_red">* </span><span>工单期限</span>
		 <input class="easyui-datetimebox" id="dateOrderTerm" name="dateOrderTerm" value="${tbOrders.dateOrderTerm}" editable="false"  style="width:200px;height:26px;">
			

<!-- 		 <input type="text" id="dateOrderTerm" name="dateOrderTerm" value="1111" > -->
	</div>
	<br />
	<div>
		...
		<c:forEach items="${taskTypeList}" var="m">
			<span class="redio_span"> <input class="redio_input"
				type="radio" id="taskType${m.value }" name="vacOrderTaskType"
				value="${m.value }"
				<c:if test="${tbOrders.vacOrderTaskType == m.value}">checked="checked"</c:if> />${m.label }
			</span>
		</c:forEach>
	</div>
	<br />
	<div>
		...
		<input class="easyui-combobox combobox_input"
			name="numOrderContactProvinceId" id="numOrderContactProvinceId" 
			value="${tbOrders.numOrderContactProvinceId }"> <input
			class="easyui-combobox combobox_input" name="numOrderContactCityId" 
			id="numOrderContactCityId" value="${tbOrders.numOrderContactCityId }">
		<input class="easyui-textbox " id="vacOrderContactAddress"
			name="vacOrderContactAddress"
			value="${tbOrders.vacOrderContactAddress}"  style="width:300px;height:26px;" >
	</div>
	<br />
	<div>
		<span class="div_margin_left " >常住地址</span>
		<input class="easyui-combobox combobox_input"
			name="numClientProvinceId" id="numClientProvinceId" 
			value="${tbClients.numClientProvinceId }"> <input
			class="easyui-combobox combobox_input" name="numClientCityId" 
			id="numClientCityId" value="${tbClients.numClientCityId }">
		<input class="easyui-textbox " id="vacClientAddress"
			name="vacClientAddress" value="${tbClients.vacClientAddress}"
	...
	<div>
		...
		<input class="easyui-combobox combobox_input"
			id="numOrderIncidentProvinceId" name="numOrderIncidentProvinceId" 
			value="${tbOrders.numOrderIncidentProvinceId }"> <input
			class="easyui-combobox combobox_input" id="numOrderIncidentCityId" 
			name="numOrderIncidentCityId"
			value="${tbOrders.numOrderIncidentCityId }"> <input
			class="easyui-textbox " id="vacOrderIncidentAddress"
			name="vacOrderIncidentAddress"
			value="${tbOrders.vacOrderIncidentAddress}"  style="width:300px;height:26px;" >
	</div>
	<br /> <br /> <span class="div_margin_left span_title">咨询内容</span>
	<p />
	<div class="div_margin_left demo_line_02"></div>
	<p />
	<div style="vertical-align: top; display: inline-flex;">
		<span class="div_margin_left textbox_title_1">* </span><span
			class="textbox_title_2">事项说明</span> <input class="easyui-textbox "
			name="vacOrderAccount" data-options="multiline:true" required="required" validType="length[0,2000]" 
			value="${tbOrders.vacOrderAccount }"
			style="width: 300px; height: 100px"> <span
			class="div_margin_left textbox_title_1">* </span><span
			class="textbox_title_2">处理意见</span> <input class="easyui-textbox " required="required"
			name="vacOrderSuggestion" data-options="multiline:true" validType="length[0,2000]" 
			value="${tbOrders.vacOrderSuggestion }"
			style="width: 300px; height: 100px">
	</div>
	<br /> <br /> <br /> <span class="div_margin_left span_title">接收区域</span>
	<p />
	<div class="div_margin_left demo_line_02"></div>
	<p />
	<div>
		...
		<input class="easyui-combobox combobox_input"
			name="numOrderReceiveProvinceId" id="numOrderReceiveProvinceId" 
			value="${tbOrders.numOrderReceiveProvinceId }" required="required"> <input
			class="easyui-combobox combobox_input" name="numOrderReceiveCityId" 
			id="numOrderReceiveCityId" value="${tbOrders.numOrderReceiveCityId }" required="required">
	</div>
	<br /> <br /> <span class="div_margin_left span_title">补充</span>
	<p />
	<div class="div_margin_left demo_line_02"></div>
	<p />
	<div>
		<span class="div_margin_left">客户其他联系电话</span> <input
			class="easyui-textbox textbox_input" name="vacOrderOtherContacts"  validType="length[0,20]" 
			value="${tbOrders.vacOrderOtherContacts }" /> <span
			class="div_margin_left  ">服务律师姓名</span> <input
			class="easyui-textbox textbox_input" name="vacOrderLawyerName"  validType="length[0,20]" 
			value="${tbOrders.vacOrderLawyerName }" /> 
				</div>
				<br/>
				<div>	<span
	...
	<br /> 
				...
				<br/><br /> <span class="div_margin_left span_title">审核信息</span>
	<p />
	<div class="div_margin_left demo_line_02"></div>
	<p />
	<div>
	...
	<div>
		<span class="div_margin_left  textbox_title_2">初审不通过原因</span> <input
			...
			value="${tbOrders.noPassReason }" data-options="multiline:true"  style="width:300px;height:100px"  validType="length[0,50]" />
	</div>
	<br /> <br />
	<div class="div_margin_left">
		<permission:hasPermission action="tb/order/saveOrder">
		<a href="javascript:saveOrder();" id="orderadd-save"
			data-options="iconCls:'icon-save'"
			class="easyui-linkbutton a_button">保 存 工 单</a>&nbsp;&nbsp;&nbsp;&nbsp;
