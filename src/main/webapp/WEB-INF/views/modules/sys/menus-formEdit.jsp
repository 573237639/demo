...
$(function(){
	...
	setTimeout(function(){
		$('#parentMenuName').combotree('setValue',${menu.parentMenu.id });
	},500);
	
	<c:if test="${not empty menu.icon}">
		$(":radio[name='icon'][value='${menu.icon}']").attr("checked",true);
	</c:if>
});
</script>
<form id="inputForm" class="margin-top20" action="${ctx}/sys/menus/edit" method="post" data-options="novalidate:true" style="height:90%;">
	...
	<table class="line-height30" style="width:98%;">
		<tr>
			...
			<td style="width:70%;padding-left:30px;">
				<select id="parentMenuName" name="parentMenuName" class="easyui-combotree" value="${menu.parentMenu.id }" style="width:200px;"   
        data-options="url:'${ctx}/sys/menus/authorMenuTree?pnode=1',required:true"></select>
			</td>
		</tr>
		...
		<tr>
			...
			<td style="width:70%;padding-left:30px;">
				<input class="easyui-textbox add-textBoxW form-control input-sm" data-options="required:true" name="name" id="name" value="${menu.name }"/>
			</td>
		</tr>
		<tr style="height:20px;"></tr>
		<tr>
		...
		<tr id="space-id-4" style="height:20px;${menu.menuType eq 1 ? 'display:none' : ''}"></tr>
		<tr id="form-id-group" style="${menu.menuType eq 1 ? 'display:none' : ''}">
			<td class="text-align-right" style="width:30%"><label>菜单动作:</label></td>
			<td style="width:70%;padding-left:30px;">
				<input class="easyui-textbox add-textBoxW form-control input-sm" data-options="" name="actName" id="actName" value="${menu.sysAct.actName }"/>
			</td>
		</tr>
		<tr style="height:20px;"></tr>
		<tr>
			...
			<td style="width:70%;padding-left:30px;">
				<label id="showSelectIcon">
					<c:if test="${not empty menu.icon}">
						<img src='${ctx}/static/easyui/ext/style/images/extjs_icons/${menu.icon}.png'>
					</c:if>
		...
		<tr>
			...
			<td style="width:70%;padding-left:30px;">
				<input class="easyui-textbox add-textBoxW form-control input-sm" data-options="" name="remarks" id="remarks" value="${menu.remarks }"/>
			</td>
		</tr>
		<tr style="height:20px;"></tr>
		<tr>
